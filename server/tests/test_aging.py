...
from aeon.core.aging import age_weight
...
class TestScoreImportance:
    def test_returns_none_when_llm_disabled(self):
        from aeon.core.llm import score_importance
    ...
    def test_parses_plain_float(self):
        ...
        with patch("aeon.core.llm.generate_search_text", return_value="0.7"):
    ...
    def test_parses_float_with_surrounding_text(self):
        from aeon.core.llm import score_importance
        from unittest.mock import patch
        with patch("aeon.core.llm.generate_search_text", return_value="Importance: 0.85"):
            cfg = _config(llm_enabled=True, llm_provider="lmstudio")
            result = score_importance("some memory", cfg)
    ...
    def test_returns_none_on_unparseable_output(self):
        from aeon.core.llm import score_importance
        from unittest.mock import patch
        with patch("aeon.core.llm.generate_search_text", return_value="I cannot rate this."), \
             patch("aeon.core.llm.generate_text", return_value=None):
            cfg = _config(llm_enabled=True, llm_provider="lmstudio")
            result = score_importance("some memory", cfg)
    ...
    def test_clamps_to_zero_one(self):
        from aeon.core.llm import score_importance
        from unittest.mock import patch
        with patch("aeon.core.llm.generate_search_text", return_value="1.0"):
            cfg = _config(llm_enabled=True, llm_provider="lmstudio")
    ...
    def test_uses_search_model_first(self):
        ...
        with patch("aeon.core.llm.generate_search_text", return_value="0.6") as mock_search, \
             patch("aeon.core.llm.generate_text") as mock_main:
            cfg = _config(llm_enabled=True, llm_provider="lmstudio")
...
class TestIngestLLMImportance:
    def test_llm_score_used_for_promotion(self, tmp_path):
        ...
        with patch("aeon.core.ingest.score_importance", return_value=0.9):
    ...
    def test_rule_based_used_when_llm_returns_none(self, tmp_path):
        ...
        with patch("aeon.core.ingest.score_importance", return_value=None):
            result = ingest(text, config=cfg)
...
class TestReflectAgingWeightedSampling:
    ...
    def test_weighted_sample_prefers_recent(self):
        from aeon.core.reflect import _select_reflection_sources
        from aeon.core.memory_store import MemoryStore
        from unittest.mock import patch
