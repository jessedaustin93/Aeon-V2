...
from aeon.core import Config, ingest_image_bytes, ingest_image_data_url, search
...
def test_image_ingestion_stores_file_json_and_vault_note(tmp_path):
    ...
    with patch("aeon.core.media.generate_image_description", return_value="A tiny test image."):
...
def test_image_ingestion_is_searchable_by_description(tmp_path):
    cfg = Config(tmp_path)
    with patch("aeon.core.media.generate_image_description", return_value="Screenshot of a dashboard with memory counts."):
