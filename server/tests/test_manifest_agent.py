...
from aeon.core.manifest_agent import _normalise, _validate_tool_entry, MANIFEST_PATH
...
class TestScanImports:
    ...
    def test_excludes_own_package(self, cfg):
        _write_py_file(cfg, "from aeon.core import Config\n")
        agent = ManifestAgent(cfg)
        imports = agent.scan_imports()
        assert "aeon.core" not in imports
...
class TestProposeToolAddition:
    ...
    def test_tool_addition_type_accepted_by_schema(self):
        from aeon.core import validate_staging_proposal
