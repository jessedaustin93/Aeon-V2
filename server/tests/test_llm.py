...
from aeon.core import Config, generate_text, ingest, reflect, simulate_action
...
from aeon.core.tasks import TaskStore


SRC_DIR = Path(__file__).parent.parent / "src" / "aeon" / "core"
...
def test_generate_text_returns_none_when_lmstudio_unavailable():
    ...
    with patch("aeon.core.llm._call_lmstudio_messages", return_value=None):
...
def test_generate_image_description_uses_vision_model_and_no_reasoning(tmp_path):
    ...
    with patch("aeon.core.llm._call_lmstudio_messages", return_value="vision ok") as call:
...
def test_generate_image_description_falls_back_to_native_endpoint(tmp_path):
    ...
    with patch("aeon.core.llm._call_lmstudio_messages", return_value=None), \
         patch("aeon.core.llm._call_lmstudio_native_image", return_value="native ok") as native:
        result = generate_image_description(image, "describe", cfg)
...
def test_detect_lmstudio_vision_model_from_loaded_models(monkeypatch):
    ...
    monkeypatch.setattr("aeon.core.llm.urllib.request.urlopen", lambda *_, **__: FakeResponse())
...
def test_resolve_lmstudio_vision_model_prefers_explicit_config(monkeypatch):
    monkeypatch.setattr("aeon.core.llm._detect_lmstudio_vision_model", lambda _: "vendor/vision-model-vl")
    cfg = Config()
...
def test_reflection_uses_mocked_llm_output(tmp_path):
    ...
    with patch("aeon.core.reflect.generate_text", return_value=_MOCK_REFLECTION_LLM):
        result = reflect(config=config)
...
def test_reflection_fallback_when_llm_returns_empty(tmp_path):
    ...
    with patch("aeon.core.reflect.generate_text", return_value=""):
        result = reflect(config=config)
...
def test_simulation_uses_mocked_llm_output(tmp_path):
    ...
    with patch("aeon.core.simulate.generate_text", return_value=_MOCK_SIM_LLM):
        result = simulate_action(tasks[0], config=config)
